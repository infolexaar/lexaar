import React from "react";
import { useLanguage } from "../contexts/LanguageContext";
import "./ComingSoonOverlay.css";

type ComingSoonOverlayProps = { className?: string; showMobileBadge?: boolean };

const COPY: Record<"RO" | "EN" | "RU", { title: string; lines: [string, string] }> = {
  RO: {
    title: "IN CURÂND",
    lines: ["Această colectie este în curs de completare.", "Revenim în curand cu noutăti."],
  },
  EN: {
    title: "COMING SOON",
    lines: ["This collection is currently being completed.", "We’ll be back soon with new updates."],
  },
  RU: {
    title: "СКОРО",
    lines: ["Эта коллекция находится в процессе заполнения.", "Скоро вернёмся с новинками."],
  },
};

const ComingSoonOverlay: React.FC<ComingSoonOverlayProps> = ({ className, showMobileBadge }) => {
  const { language } = useLanguage();
  const copy = COPY[language];

  return (
    <div className={`coming-soon-overlay ${className || ""}`.trim()} aria-hidden="true">
      <div className="coming-soon-overlay__glass" />
      {showMobileBadge && (
        <div className="coming-soon-overlay__badge" aria-hidden="true">
          <svg
            className="coming-soon-overlay__badge-icon"
            fill="#ffffff"
            viewBox="0 0 512 512"
            xmlns="http://www.w3.org/2000/svg"
          >
            <g>
              <g>
                <path d="M494.86,367.136h-0.553V322.35c0-111.772-78.086-208.648-186.536-232.659c-0.576-5.779-3.05-11.155-7.161-15.41 c-4.941-5.114-11.578-7.931-18.69-7.931H230.08c-7.112,0-13.749,2.817-18.69,7.931c-4.11,4.255-6.585,9.631-7.161,15.41 C95.779,113.702,17.693,210.578,17.693,322.35v44.786H17.14c-9.452,0-17.14,7.689-17.14,17.14v11.278 c0,7.463,4.721,14.02,11.747,16.317C78.368,433.654,165.113,445.65,256,445.65s177.632-11.996,244.253-33.779 c7.026-2.297,11.747-8.854,11.747-16.317v-11.278C512,374.825,504.311,367.136,494.86,367.136z M223.32,85.806 c1.787-1.849,4.188-2.869,6.76-2.869h51.839c2.572,0,4.973,1.018,6.76,2.869c1.787,1.85,2.723,4.285,2.634,6.855l-7.626,221.166 c-0.176,5.09-4.302,9.077-9.394,9.077h-36.585c-5.092,0-9.218-3.987-9.394-9.076l-7.626-221.166 C220.598,90.09,221.533,87.656,223.32,85.806z M495.413,395.555c0,0.223-0.097,0.48-0.314,0.551 C430.097,417.358,345.183,429.063,256,429.063S81.903,417.358,16.902,396.104c-0.217-0.071-0.314-0.327-0.314-0.55v-11.278 c0-0.305,0.248-0.553,0.553-0.553H335.62c4.58,0,8.294-3.713,8.294-8.294c0-4.58-3.713-8.294-8.294-8.294H34.281V322.35 c0-83.355,46.677-157.8,117.156-195.572l17.996,125.979c0.592,4.137,4.139,7.122,8.201,7.122c0.39,0,0.785-0.028,1.183-0.084 c4.535-0.648,7.686-4.849,7.038-9.383l-18.75-131.258c11.941-5.228,24.463-9.437,37.467-12.521l7.165,207.768 c0.485,14.069,11.893,25.091,25.972,25.091h36.585c14.078,0,25.486-11.022,25.972-25.091l7.165-207.767 c13.003,3.085,25.526,7.294,37.467,12.521l-18.75,131.258c-0.647,4.535,2.504,8.735,7.038,9.383 c0.398,0.056,0.793,0.084,1.183,0.084c4.061,0,7.609-2.986,8.201-7.122l17.996-125.979 c70.479,37.771,117.156,112.215,117.156,195.571v44.786h-53.635c-4.58,0-8.294,3.713-8.294,8.294c0,4.58,3.713,8.294,8.294,8.294 h70.775c0.305,0,0.553,0.248,0.553,0.553V395.555z" />
              </g>
            </g>
          </svg>
          <div className="coming-soon-overlay__badge-text">{copy.title}</div>
        </div>
      )}
      <div className="coming-soon-overlay__content">
        <svg
          className="coming-soon-overlay__icon"
          fill="#ffffff"
          viewBox="0 0 512 512"
          xmlns="http://www.w3.org/2000/svg"
        >
          <g>
            <g>
              <path d="M494.86,367.136h-0.553V322.35c0-111.772-78.086-208.648-186.536-232.659c-0.576-5.779-3.05-11.155-7.161-15.41 c-4.941-5.114-11.578-7.931-18.69-7.931H230.08c-7.112,0-13.749,2.817-18.69,7.931c-4.11,4.255-6.585,9.631-7.161,15.41 C95.779,113.702,17.693,210.578,17.693,322.35v44.786H17.14c-9.452,0-17.14,7.689-17.14,17.14v11.278 c0,7.463,4.721,14.02,11.747,16.317C78.368,433.654,165.113,445.65,256,445.65s177.632-11.996,244.253-33.779 c7.026-2.297,11.747-8.854,11.747-16.317v-11.278C512,374.825,504.311,367.136,494.86,367.136z M223.32,85.806 c1.787-1.849,4.188-2.869,6.76-2.869h51.839c2.572,0,4.973,1.018,6.76,2.869c1.787,1.85,2.723,4.285,2.634,6.855l-7.626,221.166 c-0.176,5.09-4.302,9.077-9.394,9.077h-36.585c-5.092,0-9.218-3.987-9.394-9.076l-7.626-221.166 C220.598,90.09,221.533,87.656,223.32,85.806z M495.413,395.555c0,0.223-0.097,0.48-0.314,0.551 C430.097,417.358,345.183,429.063,256,429.063S81.903,417.358,16.902,396.104c-0.217-0.071-0.314-0.327-0.314-0.55v-11.278 c0-0.305,0.248-0.553,0.553-0.553H335.62c4.58,0,8.294-3.713,8.294-8.294c0-4.58-3.713-8.294-8.294-8.294H34.281V322.35 c0-83.355,46.677-157.8,117.156-195.572l17.996,125.979c0.592,4.137,4.139,7.122,8.201,7.122c0.39,0,0.785-0.028,1.183-0.084 c4.535-0.648,7.686-4.849,7.038-9.383l-18.75-131.258c11.941-5.228,24.463-9.437,37.467-12.521l7.165,207.768 c0.485,14.069,11.893,25.091,25.972,25.091h36.585c14.078,0,25.486-11.022,25.972-25.091l7.165-207.767 c13.003,3.085,25.526,7.294,37.467,12.521l-18.75,131.258c-0.647,4.535,2.504,8.735,7.038,9.383 c0.398,0.056,0.793,0.084,1.183,0.084c4.061,0,7.609-2.986,8.201-7.122l17.996-125.979 c70.479,37.771,117.156,112.215,117.156,195.571v44.786h-53.635c-4.58,0-8.294,3.713-8.294,8.294c0,4.58,3.713,8.294,8.294,8.294 h70.775c0.305,0,0.553,0.248,0.553,0.553V395.555z" />
            </g>
          </g>
          <g>
            <g>
              <path d="M388.701,367.136h-17.694c-4.58,0-8.294,3.713-8.294,8.294c0,4.58,3.713,8.294,8.294,8.294h17.694 c4.58,0,8.294-3.713,8.294-8.294C396.995,370.849,393.282,367.136,388.701,367.136z" />
            </g>
          </g>
          <g>
            <g>
              <path d="M193.439,303.484l-2.529-17.698c-0.648-4.535-4.846-7.686-9.383-7.038c-4.535,0.648-7.686,4.849-7.038,9.383l2.528,17.698 c0.592,4.137,4.139,7.122,8.201,7.122c0.39,0,0.785-0.028,1.183-0.084C190.936,312.219,194.087,308.018,193.439,303.484z" />
            </g>
          </g>
          <g>
            <g>
              <path d="M330.474,278.748c-4.535-0.649-8.735,2.504-9.383,7.038l-2.528,17.698c-0.647,4.535,2.504,8.735,7.038,9.383 c0.397,0.056,0.793,0.084,1.183,0.084c4.061,0,7.609-2.986,8.201-7.122l2.528-17.698 C338.159,283.597,335.008,279.396,330.474,278.748z" />
            </g>
          </g>
        </svg>

        <div className="coming-soon-overlay__text">
          <div className="coming-soon-overlay__title">{copy.title}</div>
          <div className="coming-soon-overlay__subtitle">{copy.lines[0]}</div>
          <div className="coming-soon-overlay__subtitle">{copy.lines[1]}</div>
        </div>
      </div>
    </div>
  );
};

export default ComingSoonOverlay;

